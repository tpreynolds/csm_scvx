...
function dynamics(t::Float64,x,u,t_grid,pars::Parameters)
	r = x[pars.id_r]
	v = x[pars.id_v]
...
function dynamics(t::Float64,x,u,pars::Parameters)
	r = x[pars.id_r]
	v = x[pars.id_v]
...
function drag(v,pars::Parameters)
...
pars = Parameters(m,cd,Sd,ρ,id_r,id_v);
...
ctrl = ScvxParameters(N,Nsub,nx,nu,iter_max,wvc,wtr,wtrp,cvrg_tol,feas_tol);

println(bnds.init.t_min)

tspan = range(t0_min,stop=tf_min,length=10);
