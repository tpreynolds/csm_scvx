...
function ISS_koz(kozN::Integer)
  btms_lft, tops_rgt = repeat([zeros(3)], kozN), repeat([zeros(3)], kozN)
  	btms_lft[1], tops_rgt[1] = Vector([ 11.9,2.7,0.0 ]), Vector([ 12.0,12.0,6.2 ])  	# 1
	btms_lft[2], tops_rgt[2] = Vector([ 11.6,1.2,0.0 ]), Vector([ 12.0,2.7,6.2 ])  		# 2
	btms_lft[3], tops_rgt[3] = Vector([ 11.6,0.8,0.0 ]), Vector([ 12.0,1.2,6.2 ])  		# 3
	btms_lft[4], tops_rgt[4] = Vector([ 11.6,-12.0,0.0 ]), Vector([ 12.0,-0.8,6.2 ])  	# 4
	btms_lft[5], tops_rgt[5] = Vector([ -12.0,-12.0,0.0 ]), Vector([ 11.6,-2.7,6.2 ])  	# 5
	btms_lft[6], tops_rgt[6] = Vector([ -12.0,-2.7,0.0 ]), Vector([ 10.3,-1.2,6.2 ])  	# 6
	btms_lft[7], tops_rgt[7] = Vector([ -12.0,-1.2,0.0 ]), Vector([ 7.7,-0.6,6.2 ])  	# 7
	btms_lft[8], tops_rgt[8] = Vector([ -12.0,-0.6,0.0 ]), Vector([ 5.9,12.0,6.2 ])  	# 8
	btms_lft[9], tops_rgt[9] = Vector([ 5.9,0.6,0.0 ]), Vector([ 7.7,12.0,6.2 ])  		# 9
	btms_lft[10], tops_rgt[10] = Vector([ 9.6,7.3,0.0 ]), Vector([ 11.9,12.0,6.2 ])  	# 10
	btms_lft[11], tops_rgt[11] = Vector([ 7.7,2.7,0.0 ]), Vector([ 9.6,12.0,6.2 ])  	# 11
	btms_lft[12], tops_rgt[12] = Vector([ 7.7,1.2,0.0 ]), Vector([ 10.2,2.7,6.2 ])  	# 12
	#
	btms_lft[13], tops_rgt[13] = Vector([ 9.6,2.7,0.0 ]), Vector([ 11.9,7.3,3.8 ])  	# 1b
	btms_lft[14], tops_rgt[14] = Vector([ 9.6,2.7,5.9 ]), Vector([ 11.9,7.3,6.2 ])  	# 1t
	btms_lft[15], tops_rgt[15] = Vector([ 10.2,1.2,0.0 ]), Vector([ 11.6,2.7,4.2 ])		# 2b
	btms_lft[16], tops_rgt[16] = Vector([ 10.2,1.2,5.5 ]), Vector([ 11.6,2.7,6.2 ])		# 2t
	btms_lft[17], tops_rgt[17] = Vector([ 11.6,-0.8,0.0 ]), Vector([ 12.0,0.8,4.1 ])	# 3b
	btms_lft[18], tops_rgt[18] = Vector([ 11.6,-0.8,5.5 ]), Vector([ 12.0,0.8,6.2 ])	# 3t
	btms_lft[19], tops_rgt[19] = Vector([ 10.3,-2.7,0.0 ]), Vector([ 11.6,-1.2,4.3 ])	# 4b
	btms_lft[20], tops_rgt[20] = Vector([ 10.3,-2.7,5.4 ]), Vector([ 11.6,-1.2,6.2 ])	# 4t
	btms_lft[21], tops_rgt[21] = Vector([ 7.7,-1.2,0.0 ]), Vector([ 11.6,1.2,3.7 ])		# 5b
	btms_lft[22], tops_rgt[22] = Vector([ 7.7,-1.2,5.9 ]), Vector([ 11.6,1.2,6.2 ])		# 5t
	btms_lft[23], tops_rgt[23] = Vector([ 5.9,-0.6,0.0 ]), Vector([ 7.7,0.6,4.3 ])		# 6b
	btms_lft[24], tops_rgt[24] = Vector([ 5.9,-0.6,5.4 ]), Vector([ 7.7,0.6,6.2 ])		# 6t
...
# function ISS_kiz(kiz::Integer)
# 	btms_lft, tops_rgt = repeat([zeros(3)], kizN), repeat([zeros(3)], kizN)
#     btms_lft[1], tops_rgt[1]   = Vector([ 9.6,2.7,3.8 ]), Vector([ 11.9,7.3,5.9 ])  	# 1
#   	btms_lft[2], tops_rgt[2]   = Vector([ 10.2,1.2,4.2 ]), Vector([ 11.6,2.7,5.5 ])  	# 2
#   	btms_lft[3], tops_rgt[3]   = Vector([ 11.6,-0.8,4.1 ]), Vector([ 12.0,0.8,5.5 ])  	# 3
#   	btms_lft[4], tops_rgt[4]   = Vector([ 10.3,-2.7,4.3 ]), Vector([ 11.6,-1.2,5.4 ])  	# 4
#   	btms_lft[5], tops_rgt[5]   = Vector([ 7.7,-1.2,3.7 ]), Vector([ 11.6,1.2,5.9 ])  	# 5
# 	btms_lft[6], tops_rgt[6]   = Vector([ 5.9,-0.6,4.3 ]), Vector([ 7.7,0.6,5.4 ])  	# 6
#
# 	keepin_zones = []
# 	for (btm, top) in zip(btms_lft, tops_rgt)
# 		center, width = (top+btm)/2., (top-btm)
...
function signed_distance(r::Vector,obs::PolygonalObstacle)
  ...
  if inside
	# unit vector pointing from center to r
	v = (r-cntr)/norm(r-cntr)
    # find cos(angle) between unit vector v and polytope normals
    projs = zeros(6)
    projs[1], projs[2], projs[3] = dot(v,n1), dot(v,n2), dot(v,n3)
    projs[4], projs[5], projs[6] = dot(v,n4), dot(v,n5), dot(v,n6)
    id = argmax(projs)
    if id==1
